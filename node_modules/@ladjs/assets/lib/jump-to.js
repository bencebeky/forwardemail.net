...
const jumpTo = (target, ev) => {
  if (!isSANB(target) || target === '#') {
    ...
    if (ev && ev.currentTarget) {
      ...
      if (!$currentTarget.get(0).hasAttribute('data-target')) window.scrollTo(0, 0);
    }
  ...
  const $target = $(target);
  if ($target.length === 0) return; // Remove id and then add it back to prevent scroll
  ...
  if ($('.navbar.fixed-top').length > 0) offsetTop -= $('.navbar.fixed-top').outerHeight(); // add 20px padding to top
